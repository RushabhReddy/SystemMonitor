...
long LinuxParser::ActiveJiffies(int pid) { 
  ...
  if (filestream.is_open()) {
    ...
    for ( int i = 1; i < 18; i++){
        ...
        else if (i == 15) stime = stol(value);
        else if (i == 16) cutime = stol(value);
        else if (i == 17) cstime = stol(value);       
    }
...
long LinuxParser::ActiveJiffies() { 
  ...
  return stol(cpu_utilization[kUser_]) + stol(cpu_utilization[kNice_]) + stol(cpu_utilization[kSystem_]) + stol(cpu_utilization[kIRQ_]) + 
